            ...
            return  (<aside style={{height: '100vh'}} className="bg-[#2c190b]  w-[250px] rounded pl-5 min-h-10"> 
                        ...
                        <div>
                            {projectState.projects.map((project,index)=>{
                                return   <Button onClick={handleProjectSelection} key={index} id={project.id} className="pb-1 pl-2 w-[95%] text-white text-left font-semibold bg-opacity-10 bg-slate-300">{project.title}</Button>
                            })}
                        </div>
                    </aside>)
}